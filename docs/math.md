...
\begin{equation}
\log L = \sum_{r \le s} \Big[
L^+_{rs}\log p^+_{rs} + L^-_{rs}\log p^-_{rs} + L^0_{rs}\log p^0_{rs}
\Big].
\end{equation}
...
\begin{align}
p^+_{ij} &= \frac{x_i^+ x_j^+ \chi^+_{c_i c_j}}
...
\begin{equation}
\log L = \sum_{i<j} \Big[ w_{ij}\log z_{ij} - (w_{ij}+1)\log(1+z_{ij}) \Big].
\end{equation}
